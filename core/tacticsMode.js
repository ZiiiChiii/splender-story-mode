...
const TX_CHAPTERS = [
  {
    ...
    storyBefore: [
      ...
      { who: '赫克特', side: 'ally', text: '灰鴉傭兵團。拿人錢財替人守礦,守的還是別人的礦。守備隊人手不足,這一仗——由你來調度,我的盾聽你的。' },
      { who: '你', side: 'ally', text: '這座村子收留了不知來歷的我,礦坑就是它貧窮的病根。今天,把它連根拔了。' },
      { who: '貞德', side: 'ally', text: '礦工們斷了生計,信仰不能當飯吃。清出一條路吧,指揮官——費用照算,戰利品歸我們。' },
    ...
    storyAfter: [
      { who: '赫克特', side: 'ally', text: '礦道清出來了。這下鐵匠鋪的黑曜石訂單有著落了……欠你們一次,我的盾隨時為你們而舉。' },
  ...
  },
  {
    ...
    storyBefore: [
      ...
      { who: '亞瑟', side: 'ally', text: '哦?親自帶隊馳援?你就是在我晚宴上限時交出成績的那個商人吧。有點意思——讓本子爵看看,算盤打得響的人,刀子快不快。', req: { mainLv: 6 } },
      { who: '赫克特', side: 'ally', text: '又是灰鴉。紅岩礦道那筆帳還沒跟他們算清,正好——連本帶利。', req: { clearedCh: 1 } },
      { who: '你', side: 'ally', text: '糧倉一燒,橡木鎮的商路就斷了——和礦坑是同一個手法。各位,今晚一粒米都不能讓他們碰!' },
  ...
  },
  {
    ...
    storyBefore: [
      ...
      { who: '萊戈拉斯', side: 'ally', text: '你就是那個在戒嚴中還能準時交付軍需的商人?是個信人。為了這份信,我允許你的部隊踏入精靈的林地。', req: { mainLv: 14 } },
      ...
      { who: '你', side: 'ally', text: '價碼再高,也高不過信譽。今天折了你的翅膀,明天——我親自去首都,找那位「大人物」收帳。', mood: 'shout' },
      { who: '貞德', side: 'ally', text: '首都的大人物……很好,又一條線索。先打贏這仗,指揮官的帳,我們陪你一起收。' },
    ],
    storyAfter: [
...
export const TacticsMode = {
  ...
  /* 對外:供地圖劇情事件(storyEvents)取用同一套角色立繪 */
  portraitOf(who, side) { return this._castPortrait({ who, side: side || 'ally' }); },
  _castPortrait(line) {
    // 主角:沿用城鎮行走圖的像素立繪(跨模組同一形象)
    if (line.who === '你' && window.TownMode && window.TownMode.heroPortraitURL) {
      const u = window.TownMode.heroPortraitURL();
      return u ? { img: u, pixel: true } : {};
    }
    const CAST = {
      '貞德':   { sprite: 'joan' },
      '赫克特': { sprite: 'hector' },
