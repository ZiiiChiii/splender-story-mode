...
(function () {
  ...
  const EVENTS = [
    ...
    {
      id: 'arc2_call',
      // 第二部開幕:灰鴉篇打完 + 主線第 15 關通關(戰4閘門到位)→ 黑市告急
      cond: () => txCleared().includes(3) && mainProg().cleared.includes(15) && !txCleared().includes(4),
      headline: '第二部・影盟篇 開幕',
      subline: '微光村・廣場',
      script: () => [
  ...
  const StoryEvents = {
    ...
    /* 下一步指引(長老與佈告欄共用的導航邏輯,雙線交錯、永不誤導)
       rec = 各戰役的「主線硬性需求」(與 TacticsMode.CH_MAIN_REQ 一致):
       主線到位且該戰未打 → 指向出城;否則指向殿堂。主線的閘門(m4/m7/m15-22)
       亦以戰役通關為條件,故本表同時是唯一可行的推進節奏。 */
    _txPlan: [
      ...
      { ch: 4,  rec: 15, name: '黑市暗巷',   text: '查理曼的黑市遭影盟襲擊——清剿暗巷' },
      { ch: 5,  rec: 16, name: '火山熔爐',   text: '影盟要熄滅瓦肯的熔爐——守住軍備命脈' },
      { ch: 6,  rec: 17, name: '傀儡工房',   text: '鍊金傀儡被咒印駭走——助帕拉塞爾斯奪回核心' },
      { ch: 7,  rec: 18, name: '地底金庫',   text: '重甲衛正在劫掠索林的金庫——地底攔截' },
      { ch: 8,  rec: 19, name: '首都下水道', text: '神秘貨箱正運進大會堂地基——潛入攔截' },
      { ch: 9,  rec: 20, name: '王港碼頭',   text: '影盟傾巢突襲麥哲倫船隊——暴雨夜護航' },
